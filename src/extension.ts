...
async function outputScanResults(vulnerabilities: any[]) {
    ...
    // Terminal output
    const outputChannel = vscode.window.createOutputChannel('CodeWolf Security Scanner');
    outputChannel.clear();
    outputChannel.show(true);
    
    outputChannel.appendLine('🐺 ==========================================');
    outputChannel.appendLine('🐺 CODEWOLF SECURITY SCANNER RESULTS');
    outputChannel.appendLine('🐺 ==========================================');
    ...
    markdownContent += `---\n\n`;
    markdownContent += `## 🐺 About CodeWolf\n\n`;
    markdownContent += `**CodeWolf Security Scanner** - The Guardian Wolf for Developers\n\n`;
    ...
    markdownContent += `---\n\n`;
    markdownContent += `**Generated by CodeWolf v2.0.0** | [Support on Ko-fi](https://ko-fi.com/watsy) ☕\n\n`;
    markdownContent += `---\n`;
    markdownContent += `*Generated by CodeWolf Security Scanner - Your Guardian Wolf 🐺*\n`;
    
    try {
        fs.writeFileSync(reportPath, markdownContent);
        outputChannel.appendLine(`📄 Detailed report saved to: !CODEWOLF-SECURITY-REPORT.md`);
